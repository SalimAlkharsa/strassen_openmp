...
void multiply_strassen(int size, int **A, int **B, int **C)
{
    ...
    int **T1 = allocate_matrix(half, half);
    int **T2 = allocate_matrix(half, half);

#pragma omp task
    {
        add_matrix(T1, A11, A22, half, half);
        add_matrix(T2, B11, B22, half, half);
        multiply_strassen(half, T1, T2, M1);
    }

#pragma omp task
    {
        add_matrix(T1, A21, A22, half, half);
        multiply_strassen(half, T1, B11, M2);
    }

#pragma omp task
    {
        subtract_matrix(T2, B12, B22, half, half);
        multiply_strassen(half, A11, T2, M3);
    }

#pragma omp task
    {
        subtract_matrix(T2, B21, B11, half, half);
        multiply_strassen(half, A22, T2, M4);
    }

#pragma omp task
    {
        add_matrix(T1, A11, A12, half, half);
        multiply_strassen(half, T1, B22, M5);
    }

#pragma omp task
    {
        subtract_matrix(T1, A21, A11, half, half);
        add_matrix(T2, B11, B12, half, half);
        multiply_strassen(half, T1, T2, M6);
    }

#pragma omp task
    {
        subtract_matrix(T1, A12, A22, half, half);
        add_matrix(T2, B21, B22, half, half);
        multiply_strassen(half, T1, T2, M7);
    }
